...
EPOCHS = 30
LEARNING_RATE = 1e-4
LOG_DIR = './visualData'         # Where to save graphs/models
DATA_DIR = './data'           # Where images are
CHECKPOINT_PATH = './visualData/checkpoint.pth' # File to save/resume progress
...
def update_graph(loss_history, epoch):
    """Generates and saves a graph at the current epoch"""
    plt.figure(figsize=(10, 6))
    plt.plot(loss_history, label='Loss', color='#00ff00', linewidth=1)
    
    # Add trendline
    if len(loss_history) > 50:
        z = np.polyfit(range(len(loss_history)), loss_history, 1)
        p = np.poly1d(z)
        plt.plot(loss_history, p(range(len(loss_history))), "r--", label='Trend')

    plt.title(f'Training Loss Curve (Epoch {epoch})')
    plt.xlabel('Steps')
    plt.ylabel('Loss')
    plt.legend()
...
def train():
    ...
    with Live(layout, refresh_per_second=4) as live:
        for epoch in range(start_epoch, EPOCHS):
            ...
            # 1. Update Graph
            update_graph(loss_history, epoch)
            logs.append(f"[bold cyan]Graph updated: results/latest_training_graph.png[/bold cyan]")
